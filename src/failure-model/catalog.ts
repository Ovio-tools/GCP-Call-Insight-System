...
export const REMEDIATION_CATALOG: Record<ErrorCode, CatalogEntry> = {
  ...
  TECHNICIAN_NOTE_RUN_DEGRADED: {
    rootCauseCategory: 'TECHNICIAN_NOTE_RUN_DEGRADED',
    whatBroke: 'A batch run produced technician notes for fewer calls than expected.',
    likelyCause:
      'The AI returned output that did not fit the note format, or the AI service was rejecting or throttling requests during the run.',
    // The impact wording must keep saying, explicitly, that nothing else is affected. A
    // technician note is derived and optional: the call was already processed, its structured
    // record is already stored, and no call is held, retried, or dropped by this failure. The
    // ONLY consequence is that some calls have no note until the job is re-run.
    impact:
      'Some calls have no technician note for this run. Call processing, the redaction boundary, and the stored structured records are all unaffected — nothing is held, retried, or dropped, and the missing notes are regenerated by simply re-running the job.',
    remediationNow:
      'Re-run the note job; if the failure rate stays high, check the AI service status and the note prompt version before changing anything else.',
    remediationFix:
      'Fix the note prompt or schema so the model output validates, then regenerate the affected calls under a new prompt version.',
    dataSafe: true,
    callsState: 'none',
    owner: OVIO,
    runbookRef: 'runbook#technician-note-run-degraded',
  },
};
