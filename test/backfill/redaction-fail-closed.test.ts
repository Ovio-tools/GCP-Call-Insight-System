...
describe.skipIf(!hasTestDb || !hasRawTestDb)(
  'backfill redaction fail-closed (privacy boundary)',
  () => {
    let owner!: Pool;
    let app!: Pool;
    // DB-B (raw store): raw_transcripts now lives only here.
    let rawOwner!: Pool;
    const PREFIX = 'bf-fc-';
    const config = makeTestConfig();

    async function count(table: string, callId: string, from?: Pool): Promise<number> {
      const r = await (from ?? owner).query<{ count: number }>(
    ...
    it('the production ingest only seeds + tracks + enqueues — it never redacts or stores content', async () => {
      ...
      expect(await count('raw_transcripts', call.callId, rawOwner)).toBe(0);
