...
describe.skipIf(!hasTestDb)('store + mark-retention-eligible (Task 5.3)', () => {
  let owner!: Pool;
  let app!: Pool;
  ...
  const seedAtStore = async (callId: string): Promise<void> => {
    ...
    await putTranscript(app, keyProvider, { callId, transcript: 'Caller: no hot water.' });
  ...
  const one = async (
    ...
    const r = await owner.query(`SELECT * FROM ${table} WHERE call_id = $1`, [callId]);
  ...
  it('runs to completed, stamps raw + vault + candidate retention-eligible, deletes nothing', async () => {
    ...
    const raw = await one('raw_transcripts', callId);
    ...
    expect(await rowCount('token_vault', callId)).toBe(2);
    const unstampedVault = await owner.query<{ n: string }>(
    ...
    const deletedVault = await owner.query<{ n: string }>(
      `SELECT count(*)::text AS n FROM token_vault
  ...
  it('re-running mark-retention-eligible does not reset the stamp or delete anything', async () => {
    ...
    const rawBefore = (await one('raw_transcripts', callId))?.retention_eligible_at as Date;
    expect(rawBefore).not.toBeNull();

    // Re-run the final stage handler directly (the runner would no-op a completed call).
    await createMarkRetentionEligibleHandler()(ctx(callId, 'mark-retention-eligible'));

    const rawAfter = (await one('raw_transcripts', callId))?.retention_eligible_at as Date;
    expect(new Date(rawAfter).getTime()).toBe(new Date(rawBefore).getTime());
    expect(await rowCount('raw_transcripts', callId)).toBe(1);
    expect(await rowCount('token_vault', callId)).toBe(2);
  });
