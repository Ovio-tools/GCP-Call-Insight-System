...
describe.skipIf(!hasTestDb)('migration 6 precondition (pre-existing skipped rows)', () => {
  ...
  // 1782864100003 (structured_knowledge.superseded_by_call_id), 1782864100004
  // (below_minimum_duration drop reason) and 1782864100005 (technician_notes + note_feedback,
  // ADR 0009) sit above 6, so we roll
  // back 20 migrations to reach the pre-6 schema. (The drop migration's down() recreates
