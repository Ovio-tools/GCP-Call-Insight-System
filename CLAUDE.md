...
- **redact** (Task 4.1, built; precision-scoped by ADR 0006; residual-superset by
  ...
  (unsafe surface); title-case-pass detections count only for PERSON. The primary
  layers are a strict SUPERSET of the residual scan (ADR 0007): new regex detectors
  (generic ≥7-digit `number` runs, spelled-out digits, greeting-cue names, email
  confusables) plus a residual-mirror repair fixpoint inside the ONE shared
  `composeRedaction` (`src/redaction/{compose,repair,mirror-finders}.ts`) re-redact
  anything the residual could hold on — residual-scan.ts is byte-unchanged as the
  auditor, and `residual_pii_detected` is reachable only via repair-cap exhaustion.
  A residual
  ...
  corpus recall / no-egress / adversarial / precision / superset gates run in CI
