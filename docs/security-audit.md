...
| 3   | dialpad — `src/dialpad/webhook/route.ts` content-type parser | doc-only              | The receiver calls `removeAllContentTypeParsers`/`addContentTypeParser` — tokens the conformance guard flags as "self-rolled hardening".      | Confirmed legitimate: it installs a raw-**body** parser so a bare-JWT body is not rejected as malformed JSON, sitting ON TOP of `createWebhookApp` (not a bypass). Allowlisted (single documented owner) in `conformance.test.ts`; a NEW file installing a parser still fails the guard. | webhook (3.2) |
